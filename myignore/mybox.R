check_overlap <- function(p) {
  g <- ggplotGrob(p)
  a <- g$grobs[[grep('axis-b', g$layout$name)]]$children$axis$grobs[[
    2
  ]]$children[[1]]
...
easy_axis_angle <- function(p) {
  if (check_overlap(p)) p <- p + theme(axis.text.x = element_text(angle = 90))
  p
}

p0 <- ggplot2::ggplot(iris, ggplot2::aes(x = Sepal.Length, y = Sepal.Width)) +
  geom_point()
...
x <- g$grobs[[grep('axis-b', g$layout$name)]]$children$axis$grobs[[
  2
]]$children[[1]]
...
data.frame(Box) %>%
  slice(1:2) %>%
  ggplot(aes(x = x1, xend = x2, y = y1, yend = y2)) +
  geom_segment()
...
centroid <- function(x1, y1, x2, y2, hj, vj) {
  matrix(c(x1 + (x2 - x1) * hj, y1 + (y2 - y1) * vj), ncol = 2)
}
...
Rotate <- function(x, y = NULL, mx = NULL, my = NULL, rot = 60, asp = 1) {
  theta <- rot * pi / 180
  xy <- xy.coords(x, y)
  if (is.null(mx)) mx <- mean(xy$x)
  if (is.null(my)) my <- mean(xy$y)
  dx <- xy$x - mx
  dy <- xy$y - my
